...
const ModalRouterInner = ({
  ...
  const [modalState, setModalState] = useState(() => {
    ...
    console.log("ModalRouterInner: useState modalState", { modalIsOpen, modalPath });
    return {
      modalIsOpen,
      modalPath,
      autoForce: false,
  ...
  useEffect(() => {
    ...
    if (modalState.autoForce === modalState.modalPath) {
      setModalState(prev => ({ ...prev, autoForce: false, pendingRestore: null }));
    }
    ...
    if (shouldRestore) {
      console.log("ModalRouterInner: Restoring original modal:", modalState.pendingRestore);
      setModalState(prev => ({
        ...prev,
  ...
  }, [modalState.modalPath, modalState.forced, modalState.pendingRestore, modalState.autoForce]);
  ...
  if (modalState.autoForce !== false &&
      modalState.autoForce !== modalState.modalPath &&
      modalState.forced !== modalState.modalPath) {
    console.log("ModalRouterInner: AutoForce redirect to autoforce: modalState:", modalState);
  ...
  return (
    <Routes>
      <Route
        path="*"
        element={
          <ModalRouterInnerHidrated {...{ children, hash, ...rest }} />
        }
      />
    </Routes>
  );
