...
- Use GoogleAnalytics ([integrated as a React component](https://github.com/emibcn/Rac1.js/blob/master/app/src/GAListener.jsx)) for usage statistics, but respect user's [DNT](https://en.wikipedia.org/wiki/Do_Not_Track) and don't even load it if user agent reports DNT header, unless the user explicitly opts in to tracking. Also allow users without DNT to explicitly opt out from tracking.
- Send events to GoogleAnalytics (when active) combinig [React HOC](https://reactjs.org/docs/higher-order-components.html) and [React Context](https://reactjs.org/docs/context.html) at my [GAListener](https://github.com/emibcn/Rac1.js/blob/master/app/src/GAListener/withGAEvent.js) submodule.
